...
import CatalogueAFS from './CatalogueAFS'
import Webinaires from './Webinaires'
import './AccueilPublic.css'

export default function AccueilPublic({ onLogin }) {
  const [tab, setTab] = useState('catalogue')

  return (
    <div className="accueil-public">
      <header className="accueil-header">
        <div className="accueil-header-inner">
          <div className="accueil-logo">
            <svg className="logo-svg" viewBox="-3 0 306 301" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            <span className="accueil-logo-text">pennylane</span>
            <span className="accueil-logo-sep" />
            <span className="accueil-logo-suite">Formation AFS</span>
          </div>
          <nav className="accueil-nav">
            <button
              className={`accueil-nav-btn ${tab === 'catalogue' ? 'active' : ''}`}
              onClick={() => setTab('catalogue')}
            >📚 Catalogue</button>
            <button
              className={`accueil-nav-btn ${tab === 'webinaires' ? 'active' : ''}`}
              onClick={() => setTab('webinaires')}
            >💻 Webinaires</button>
          </nav>
          <button className="accueil-login-btn" onClick={onLogin}>Se connecter</button>
        </div>
      </header>

      <main className="accueil-main">
        {tab === 'catalogue' && <CatalogueAFS publicMode={true} />}
        {tab === 'webinaires' && <Webinaires />}
      </main>
    </div>
  )
}
